...
function smartSummarize(text: string): string {
  ...
  const sub = cleaned.substring(0, 140);
  ...
  return sub.substring(0, lastSpace > 0 ? lastSpace : 140) + "...";
}
...
export async function buildUiPayload(
  ...
  const opportunities = sortedItems.map((item, index) => {
    ...
    return {
      ...
      url: item.link,
      summary: (item.ai_analysis?.executive_summary ?? smartSummarize(item.snippet)).substring(0, 140),
...
export async function writeStaticApiPayload(
  ...
  // Copy auxiliary static files (changelog, profile)
  for (const [src, dest] of [
    ["CHANGELOG.md", `${outputDir}/changelog.md`],
    ["data/mestrado_profile_requirements.json", `${outputDir}/profile.json`],
  ]) {
    try {
      await copyFile(join(process.cwd(), src), dest);
    } catch {
